...
namespace ColorLand
{
    ...
    public class Sprite
    {
        ...
        {
            draw(spritebatch, angle, destRectangle(), Vector2.Zero, new Color(R, G, B));
        }

        public void draw(SpriteBatch spritebatch, Color color)
        {
            draw(spritebatch, 0, destRectangle(), Vector2.Zero, color);
        }

        public void draw(SpriteBatch spritebatch, float angle, Rectangle rotationPosition, Vector2 origin, Color color)
        {
            if (mVisible) 
            {
                ...
                if (mIndividualImages)
                {
                    ...
                    //if(mAnimationSequence.Length>1)
                        spritebatch.Draw(mImages[imageIndex], new Rectangle((int)mX + (int)offset.X, (int)mY + (int)offset.Y, mImages[imageIndex].Width, mImages[imageIndex].Height), new Rectangle(0, 0, mImages[imageIndex].Width, mImages[imageIndex].Height), color, (float)angle, Vector2.Zero, flipEffect, 0);
                }
                else
                {
                    //spritebatch.Draw(mImage, new Rectangle((int)mX + (int)offset.X, (int)mY + (int)offset.Y, mImage.Width, mImage.Height), srcRectangle(), new Color(R, G, B), (float)angle, Vector2.Zero, SpriteEffects.None, 0);
                }
            }
        }        
